...
inline void configure(DelimiterConfig& cfg) {
    ...
    if (cfg.transparency == Transparency::BitStuffing && cfg.abortOnes != 0U) {
        ...
        if (cfg.abortOnes <= delimiterOnes) {
            throw std::invalid_argument(std::format("gr::digital::configure: abortOnes {} does not exceed the run of {} consecutive ones the delimiters carry, so the abort would fire inside the delimiter that closes a frame and abandon every frame", cfg.abortOnes, delimiterOnes));
        }
    }
