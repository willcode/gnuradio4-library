...
const boost::ut::suite<"scrambler"> scramblerTests = [] {
    ...
    "validation rejects, and leaves the configuration intact"_test = [] {
        ...
        for (const unsigned bad : {0U, 9U, 16U}) {
            expect(throws<std::invalid_argument>([&cfg, ccsds, bad] { configureScrambler(cfg, ccsds, 0xFFULL, ScramblerMode::Additive, bad, BitOrder::MsbFirst); })) << std::format("bitsPerItem = {}", bad);
        }
        expect(throws<std::invalid_argument>([&cfg, ccsds] { configureScrambler(cfg, ccsds, 0x100ULL, ScramblerMode::Additive, 8U, BitOrder::MsbFirst); })) << "a seed carrying bits above the degree";
